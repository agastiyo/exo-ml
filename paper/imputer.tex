...
In SWRF-Impute, both a lower and upper truncation point is set. The truncation bounds are the user-defined physical limits $l_p$ and $u_p$. If a bound is absent, the corresponding truncation is removed (i.e.\ $l_p = -\infty$ or $u_p = +\infty$), and the draw degenerates to a half-normal or ordinary normal as appropriate.
...
\begin{equation}
  x^{(t,k+1)}_{i,p} \sim \text{TruncNormal}\!\left(x^{pred,(t,k)}_{i,p},\; \beta\,\sigma^{(t,k)}_{p,i},\; l_p,\; u_p\right)
\end{equation}
